...
#define PH_HALHW_RC663_RAISING_EDGE_TRIGGER         0x00U     /**< Set irq0en (08h) register bit 8 to 0U to configure interrupt as raising edge trigger (active high). */
...
void phhalHw_Rc663_EventCallback(void * pDataParams)
{
#ifndef _WIN32
                                  ...
                                  )
{
    ...
    do
    {
        ...
        if(pDataParams->bJewelActivated == PH_ON)
        {
            ...
            wJewelBufferLen = wTmpBufferLen + 2U;
            wTmpBufferLen = 1;
        ...
        (( status & PH_ERR_MASK) == PH_ERR_SUCCESS_INCOMPLETE_BYTE)))
    {
        ...
        /*verify Crc_B for the received data*/
            PH_CHECK_SUCCESS_FCT(statusTmp,phTools_ComputeCrc_B( *ppRxBuffer, wTmpBufferLen-2U, aCrc));
    ...
    if((0U != (wVerifyRxCrc5)) &&  ((*pRxLength - pDataParams->wRxBufStartPos) != 0U))
    {
        ...
        *pRxLength = (pDataParams->wAdditionalInfo > 0U) ? ((bBitlen >> 3U) + 1U) : (bBitlen >> 3U);
    ...
    )
{
    ...
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_Int_ReadDerivative(pDataParams, &wDerivate, &bVersion));

    /* If Front End attached is RC663PLUS, then always apply the values to regsiter from EEPROM. */
    if ((wDerivate == PHHAL_HW_CLRC663PLUS_PRODUCT_ID) &&
        (bVersion == PHHAL_HW_CLRC663PLUS_VERSION))
    {
        pDataParams->bLoadRegMode = PH_ON;
    }
    else
    ...
    case PHHAL_HW_CARDTYPE_ISO14443A:
        if((wDerivate == PHHAL_HW_CLRC663_PRODUCT_ID1) ||
           ...
           (wDerivate == PHHAL_HW_CLRC663PLUS_PRODUCT_ID))
        {
                                   ...
                                   )
{
    ...
    const uint16_t *  PH_MEMLOC_REM pShadowDefault;
    uint16_t    PH_MEMLOC_REM wDerivative = 0;
    ...
    case PHHAL_HW_CONFIG_TXDATARATE_FRAMING:
        ...
        if(wShadowCount > 0U)
        {
            for (wIndex = 0; wIndex < wShadowCount; ++wIndex)
            {
                ...
                {
                    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(pDataParams, wConfigShadow, pShadowDefault[(wIndex << 1U) + 1U]));
                }
            }
    ...
    case PHHAL_HW_RC663_CONFIG_LPCD_CHARGEPUMP:
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_Int_ReadDerivative(pDataParams, &wDerivative, &bVersion));

        if ((wDerivative == PHHAL_HW_CLRC663PLUS_PRODUCT_ID) &&
            (bVersion == PHHAL_HW_CLRC663PLUS_VERSION))
        {
            if (wValue == PH_ON)
            {
...
static void phhalHw_Rc663_Reset(void)
{
    ...
    /* delay of ~2 ms */
    phDriver_TimerStart(PH_DRIVER_TIMER_MILLI_SECS, PHHAL_HW_RC663_RESET_DELAY_MILLI_SECS, NULL);
