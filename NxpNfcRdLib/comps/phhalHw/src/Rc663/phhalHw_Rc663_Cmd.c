                                  ...
                                  )
{
    ...
    dwReload = pDataParams->wDetectionTimeUs * PHHAL_HW_RC663_TIMER0_FREQ;
    ...
    dwReload = pDataParams->wPowerDownTimeMs * 16U;
    ...
    if ((0U == ((pDataParams->bMode & PHHAL_HW_RC663_CMD_LPCD_MODE_OPTION_IGNORE_IQ))))
    {
            ...
            (bVersion == PHHAL_HW_CLRC663PLUS_VERSION))
        {
            ...
            else
            {
                ...
                bQMax = pDataParams->bQ + 1U;
                bIMax = pDataParams->bI + 1U;
                /* Set the Min I and Q value for other Derivatives. */
                bQMin = pDataParams->bQ - 1U;
                bIMin = pDataParams->bI - 1U;
            }
        }
        else
        {
    ...
    )
{
    ...
    if((wDerivate == PHHAL_HW_SLRC610_PRODUCT_ID) &&
        (bTxProtocol >= 0x0AU))
    {
        bTxProtocol -= 0xAU;
    }
                                         ...
                                         )
{
    ...
    {
        if ((bI == 0x00U) || (bI >= 0x3FU) || (bQ == 0x00U) || (bQ >= 0x3FU))
        {
            return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
        }
    }
