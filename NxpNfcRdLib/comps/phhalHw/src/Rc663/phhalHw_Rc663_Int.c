                                   ...
                                   )
{
    ...
    if (pDataParams->bFifoSize == PH_NXPNFCRDLIB_CONFIG_RC663_VALUE_FIFOSIZE_512)
    {
        ...
        wFifoLen = wFifoLen + ((uint16_t)(bValue & PHHAL_HW_RC663_MASK_FIFOLENGTH_HI) << 8U);
    ...
    {
        /* Check if FiFO has enough space */
        if (wLength > (0xFF - wFifoLen)) // Signed
        {
            /* Truncate byte count */
                                  ...
                                  )
{
        ...
        (pDataParams->bFifoSize == PH_NXPNFCRDLIB_CONFIG_RC663_VALUE_FIFOSIZE_512))
    {
        ...
        /* If higher bits are set, we ignore the lower bits since they would change too fast in case of a slow interface. */
        wFifoLen = (uint16_t)(bValue & PHHAL_HW_RC663_MASK_FIFOLENGTH_HI) << 8U;
    }
                                  ...
                                  )
{
    ...
    if (pDataParams->bBalConnectionType == PHHAL_HW_BAL_CONNECTION_SPI)
    {
        ...
        /* Write each single data byte */
        (void)memset(pData, (int)bAddress, wLength);
        ...
        else
        {
            ...
            if ((aTempRxBuf[4] & 0xFFU) >= pDataParams->bEmdNoiseMaxDataLen)
            {
                do
                {
                    ...
                    phhalHw_Rc663_GetMultiReg(pDataParams, (uint8_t*)&aTempRxBuf);
                } while (0U == (aTempRxBuf[1] & PHHAL_HW_RC663_BIT_RXIRQ));
                break;
            }
        }
...
void phhalHw_Rc663_CreateRespFrame(uint8_t* bInBuff, uint8_t bDataPos, uint8_t * bOutBuff)
{
    ...
    /* copy full id*/
    (void)memcpy(&bOutBuff[bStartPos + 1U], &bInBuff[bDataPos], (bTemp - 1U));
...
phStatus_t phhalHw_Rc663_FrameRxMultiplePkt(phhalHw_Rc663_DataParams_t * pDataParams)
{
    ...
    if(bFrameCount > 0U)
    {
        do{
            ...
            /* clear buffer */
            (void)memset(&aRxMulFrame[0], 0x00, sizeof(aRxMulFrame));
...
phStatus_t phhalHw_Rc663_Int_ReadDerivative(phhalHw_Rc663_DataParams_t * pDataParams, uint16_t * pFEDerivate,
    uint8_t * pVersion)
{
    phStatus_t  PH_MEMLOC_REM statusTmp;
    ...
    /* If the derivative is not updated, then read the value and update in dwDerivative also. */
    if (pDataParams->dwDerivative == 0U)
    {
        ...
        wDerivate = (wDerivate << 8U) | pDerivate[0];

        *pFEDerivate = wDerivate;

        /* Read the Version register to differentiate between RC663 and RC663 Plus */
        ...
        pDataParams->dwDerivative = ((uint32_t)(*pFEDerivate) << PHHAL_HW_DERIVATIVE_POS) & PHHAL_HW_DERIVATIVE_MASK;
        pDataParams->dwDerivative |= ((uint32_t)(*pVersion) << PHHAL_HW_VERSION_POS) & PHHAL_HW_VERSION_MASK;
    }
    else
    {
        /* If the derivative is already read, then return the value as in dwDerivative. */
        *pFEDerivate = (uint16_t)(pDataParams->dwDerivative & PHHAL_HW_DERIVATIVE_MASK);
        *pVersion = (uint8_t)((pDataParams->dwDerivative & PHHAL_HW_VERSION_MASK) >> PHHAL_HW_VERSION_POS);
    }
