                                          ...
                                          )
{
    ...
    {
        pT2T->pAlT2TDataParams = pDataParams->pTopTagsDataParams[pDataParams->bTagType - 1];
    }
    else
    {
    ...
    if(0U != (pT2T->bLockTlvCount))
    {
        for(wIndex = 0; wIndex < pT2T->bLockTlvCount; wIndex++)
        {
            ...
            for(wLockBytesCount = 0;
                wLockBytesCount < (pT2T->asLockCtrlTlv[wIndex].bSizeInBits / 8u);
                wBlockNo += 4U ) {
                ...
                wTempIndex = wOffset;
                for( ; ((wLockBytesCount < (pT2T->asLockCtrlTlv[wIndex].bSizeInBits / 8U)) && (wTempIndex <= 16U));
                    wLockBytesCount++ ) {
                    if(wTempIndex != 16U)
                    {
                    ...
                    if((wTempIndex >= 4U) && ((wTempIndex % PHAL_TOP_T2T_BYTES_PER_BLOCK) == 0U))
                    {
                        PH_CHECK_SUCCESS_FCT(status, phalTop_Sw_Int_T2T_Write(
                            ...
                            &aData[(((wTempIndex / PHAL_TOP_T2T_BYTES_PER_BLOCK) - 1) * 4U)]));
                    }
    ...
    else
    {
        ...
        wDefaultLockAddress = (pT2T->bTms * 8U) + /* */ 16;
        ...
        if(((wDefaultLockLength / 8U) % PHAL_TOP_T2T_BYTES_PER_BLOCK) || (wDefaultLockLength % 8U))
        {
            ...
            while((wIndex < bDataLen) && (!bExitLoop))
            {
                ...
                case PHAL_TOP_T2T_LOCK_CTRL_TLV:
                    ...
                    if(pT2T->bTagMemoryType != PHAL_TOP_T2T_TAG_MEMORY_TYPE_STATIC)
                    {
                        ...
                        bPageAddr = (uint8_t)((aData[wIndex + 2U] & 0xF0U) >> 4U);
                        ...
                        pT2T->asLockCtrlTlv[pT2T->bLockTlvCount].wByteAddr =
                            (bPageAddr * (1U << bBytesPerPage)) + bByteOffset;
                                        ...
                                        )
{
    ...
    *pTagState = PHAL_TOP_STATE_NONE;
    ...
    /* Update tag state */
    if((pT2T->wNdefHeaderAddr) && (pDataParams->wNdefLength != 0U))
    {
    ...
    else if((pT2T->wNdefHeaderAddr) && (pDataParams->wNdefLength == 0U))
    {
        if(pT2T->bRwa == PHAL_TOP_T2T_CC_RWA_RW)
        {
                                         ...
                                         )
{
    ...
    for(wIndex = 0; ((wIndex < pT2T->bLockTlvCount) && (wIndex < PHAL_TOP_T2T_MAX_LOCK_CTRL_TLV)); wIndex++)
    {
        ...
        aTlv[(wTlvOffset++) % 8] = (uint8_t)((uint16_t)(pT2T->asLockCtrlTlv[wIndex].wByteAddr /
                                   (1U << pT2T->asLockCtrlTlv[wIndex].bBytesPerPage)) << 4U) |
                                   (uint8_t)(pT2T->asLockCtrlTlv[wIndex].wByteAddr %
                                   (1U << pT2T->asLockCtrlTlv[wIndex].bBytesPerPage));

        aTlv[(wTlvOffset++) % 8] = pT2T->asLockCtrlTlv[wIndex].bSizeInBits;
        aTlv[(wTlvOffset++) % 8] = (pT2T->asLockCtrlTlv[wIndex].bBytesLockedPerBit << 4U) |
        ...
        (void)memcpy(aTlv, &aTlv[7 - (wTlvOffset % 4U)], (wTlvOffset % 4U));
    }
    ...
    for(wIndex = 0; ((wIndex < pT2T->bMemoryTlvCount) && (wIndex < PHAL_TOP_T2T_MAX_MEM_CTRL_TLV)); wIndex++)
    {
        ...
        aTlv[(wTlvOffset++) % 8] = (uint8_t)((uint16_t)(pT2T->asMemCtrlTlv[wIndex].wByteAddr /
                                   (1U << pT2T->asMemCtrlTlv[wIndex].bBytesPerPage)) << 4U) |
                                   (uint8_t)(pT2T->asMemCtrlTlv[wIndex].wByteAddr %
                                   (1U << pT2T->asMemCtrlTlv[wIndex].bBytesPerPage));
    ...
    if((pT2T->bLockTlvCount !=0U) && (pT2T->bMemoryTlvCount != 0U))
    {
        pT2T->wNdefHeaderAddr =
            ...
            pT2T->asLockCtrlTlv[pT2T->bLockTlvCount - 1].wOffset:
        pT2T->asMemCtrlTlv[pT2T->bMemoryTlvCount - 1].wOffset) + 5U;
    }
    else if(pT2T->bLockTlvCount !=0U)
    {
        pT2T->wNdefHeaderAddr = pT2T->asLockCtrlTlv[pT2T->bLockTlvCount - 1].wOffset + 5U;
    }
    else if(pT2T->bMemoryTlvCount != 0U)
    {
        pT2T->wNdefHeaderAddr = pT2T->asMemCtrlTlv[pT2T->bMemoryTlvCount - 1].wOffset + 5U;
    }
    else
    {
                                       ...
                                       )
{
    ...
    *pLength = 0;
    ...
    wTempIndex = 0;
    for( ; wTempIndex < pDataParams->wNdefLength; wIndex += 16U)
    {
        /* Read 16 bytes */
        PH_CHECK_SUCCESS_FCT(status, phalTop_Sw_Int_T2T_Read(
            pT2T,
                                        ...
                                        )
{
    ...
    wTempIndex = wCount;
    for( ; wTempIndex < (wLength + wTernimatorTlvLen); )
    {
        /* Check if write offset is in current sector */
...
{
    if(0u != ((pT2T->sSector.bLockReservedOtp[wIndex / 8]) & (uint8_t)(1U << (wIndex % 8U))))
    {
                                             ...
                                             )
{
    ...
    else
    {
        ...
        wTagSize = ((pT2T->bTms) * 8U) + /* */ 16;

        /* Add lock TLV length */
        for(wIndex = 0; wIndex < pT2T->bLockTlvCount; wIndex++)
        {
            wTagSize += (pT2T->asLockCtrlTlv[wIndex].bSizeInBits / 8U);
        ...
        for(wIndex = 0; wIndex < pT2T->bLockTlvCount; wIndex++)
        {
            if((pT2T->asLockCtrlTlv[wIndex].wByteAddr / PHAL_TOP_T2T_SECTOR_SIZE) == pT2T->sSector.bAddress)
            {
                ...
                for(wOffset = wSectorOffset;
                    ...
                    ((pT2T->asLockCtrlTlv[wIndex].bSizeInBits / 8U) / PHAL_TOP_T2T_BYTES_PER_BLOCK) +
                    wBlockOffset);
                    wOffset++)
                {
                    pT2T->sSector.bLockReservedOtp[wOffset / 8] |= (uint8_t)(1U << (wOffset % 8U));
                }
            }
        }
        /* Update Memory Bytes */
