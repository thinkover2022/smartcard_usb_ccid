                                ...
                                )
{
    ...
    if((pDataParams->pTopTagsDataParams[pDataParams->bTagType - 1])  != NULL)
    {
        pMfcTop->pPalI14443paDataParams = pDataParams->pTopTagsDataParams[pDataParams->bTagType - 1];
    }
    else
    {
    ...
    *pTagState = PHAL_MFCTOP_STATE_NONE;
    ...
    for(bCount = 0; bCount < 5U; bCount++)
    {
        ...
        while(bMadByte < 15U)
        {
            ...
            {
                lwTemp = 1;
                lwTemp <<=  ( aNFCSectorSubstratum[bCount] + ((bMadByte - 1)/2U) );
                               ...
                               )
{
    ...
    if(status == PH_ERR_SUCCESS)
    {
        ...
        (void)memcpy(pData, (pRxBuffer + (pMfcTop->bNdefMessageStart % PHAL_MFCTOP_BLOCK_SIZE) + ((pDataParams->wNdefLength < 0xFF) ? 2 : 4)) , bFirstReadLength);
    ...
    while(0U != bReadOperations)
    {
        ...
        if((!((aCommand[1] > 0x7FU)) ?  ((aCommand[1] - 0x80U) % PHAL_MFCTOP_MAD2_SECTOR_SIZE) : (aCommand[1] % PHAL_MFCTOP_SECTOR_SIZE)))
        {
                                ...
                                )
{
    ...
    {
        bBlockNumber =(uint8_t) (0x80 + (((pMfcTop->bFirstNdefSector - 0x20) + 1U) * 16U) - (16 - (pMfcTop->bNdefMessageStart / PHAL_MFCTOP_BLOCK_SIZE)));
    }
    else
    {
        bBlockNumber = ((pMfcTop->bFirstNdefSector + 1U) * 4U) - (4 - (pMfcTop->bNdefMessageStart / PHAL_MFCTOP_BLOCK_SIZE));
    }
    ...
    if(0U == (bSpillOver))
    {
        if(0u != ((pMfcTop->bNdefMessageStart % PHAL_MFCTOP_BLOCK_SIZE)))
        {
            ...
            (void)memcpy(&aWriteLength[0], pRxBuffer, (size_t)(pMfcTop->bNdefMessageStart % PHAL_MFCTOP_BLOCK_SIZE));
        }
    ...
    if(wLength > (PHAL_MFCTOP_BLOCK_SIZE - pMfcTop->bOffset))
    {
        /* Calculate the count of Write Cycles */
        bCount = (uint8_t)((wLength - (PHAL_MFCTOP_BLOCK_SIZE - pMfcTop->bOffset)) / PHAL_MFCTOP_BLOCK_SIZE);
        bTerminatorByte = (uint8_t)((wLength - (PHAL_MFCTOP_BLOCK_SIZE - pMfcTop->bOffset)) % PHAL_MFCTOP_BLOCK_SIZE);
    ...
    if(pMfcTop->bOffset < PHAL_MFCTOP_BLOCK_SIZE)
    {
        ...
        (void)memcpy(&aWriteLength[pMfcTop->bOffset], pData, (size_t)(PHAL_MFCTOP_BLOCK_SIZE - pMfcTop->bOffset));
    }
    ...
    while(0U != bCount)
    {
        ...
        if(!((bBlockNumber > 0x7FU) ?  ((bBlockNumber - 0x80U) % PHAL_MFCTOP_MAD2_SECTOR_SIZE) : (bBlockNumber % PHAL_MFCTOP_SECTOR_SIZE)))
        {
                                 ...
                                 )
{
    ...
    do
    {
        if(bSectorCount >  0x1FU)
        {
            ...
            bBlockNo = (uint8_t) ((128u + ((bSectorCount - 0x1F) * 16U) - 1));
        }
        else
        {
            bBlockNo = (((bSectorCount + 1U) * 4U) - 1);
        }
...
{
    uint8_t PH_MEMLOC_REM bBlockNo = 0;
    uint8_t PH_MEMLOC_REM bBlockCount = 3;
    ...
    while(0u != (lwNFCSectorCount & (lwTemp << (pMfcTop->bFirstNdefSector))))
    {
        do
        {
            if(pMfcTop->bFirstNdefSector >  0x1FU)
            {
                bBlockNo = (uint8_t) ((128u + ((pMfcTop->bFirstNdefSector - 0x1F) * 16U) - 1U));
                bSectorLimit = 0x0F;
            }
            else
            {
        ...
        if(0u != (*pTagState))
        {
            ...
            pDataParams->wMaxNdefLength = (uint16_t)((PHAL_MFCTOP_SECTOR_DATA_SIZE * PHAL_MFCTOP_BLOCK_SIZE) - pMfcTop->bNdefMessageStart);
            if((pMfcTop->bFirstNdefSector + pMfcTop->bNdefSectorCount) > 31U)
            {
                pDataParams->wMaxNdefLength += (((pMfcTop->bFirstNdefSector > 16u) ? (31 - pMfcTop->bFirstNdefSector): (30 - pMfcTop->bFirstNdefSector)) *
                    PHAL_MFCTOP_SECTOR_DATA_SIZE * PHAL_MFCTOP_BLOCK_SIZE);
                pDataParams->wMaxNdefLength += (((pMfcTop->bNdefSectorCount + pMfcTop->bFirstNdefSector) - 31) *
                    PHAL_MFCTOP_MAD2_SECTOR_DATA_SIZE * PHAL_MFCTOP_BLOCK_SIZE);
            }
            else
            {
                pDataParams->wMaxNdefLength += ((pMfcTop->bNdefSectorCount - pMfcTop->bFirstNdefSector) * PHAL_MFCTOP_SECTOR_DATA_SIZE * PHAL_MFCTOP_BLOCK_SIZE);
            }
