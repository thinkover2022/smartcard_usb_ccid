                                        ...
                                        )
{
    ...
    {
        pT4T->pAlT4TDataParams = pDataParams->pTopTagsDataParams[pDataParams->bTagType - 1];
    }
    else
    {
    ...
    if(pRxBuffer[PHAL_TOP_T4T_NDEFTLV_OFFSET] == PHAL_TOP_T4T_NDEF_TLV)
    {
        ...
        if(pT4T->bRa == PHAL_TOP_T4T_NDEF_FILE_READ_ACCESS)
        {
                ...
                ((((uint16_t)pT4T->aNdefFileID[1] << 8U) | pT4T->aNdefFileID[0]) == 0x3FFFU)))
            {
                ...
                if((pDataParams->wNdefLength > 0x0000U) && (pDataParams->wNdefLength <= (pT4T->wMaxFileSize - 2)))
                {
                                       ...
                                       )
{
    ...
    *pLength = 0;
    ...
    for(wCount = 0; wNdefLen > wReadLength; )
    {
        ...
        (void)memcpy(&pData[wCount], pRxBuffer, wReadLength);
                                        ...
                                        )
{
    ...
    wWriteLength = (pT4T->wMLc < 0xFFU)? pT4T->wMLc : 0xFF;

    /* Write the NDEF message from 2nd Byte in memory */

    for(wCount = 0; wNdefLen > wWriteLength; )
    {
        wOffset = wCount + 2U;
