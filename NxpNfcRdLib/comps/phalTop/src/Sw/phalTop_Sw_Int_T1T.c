                                          ...
                                          )
{
    ...
    {
        pT1T->pAlT1TDataParams = pDataParams->pTopTagsDataParams[pDataParams->bTagType - 1];
    }
    else
    {
    ...
    {
        for(wTempIndex = 0; wTempIndex < (pT1T->asLockCtrlTlv[wIndex].bSizeInBits / 8U); wTempIndex++)
        {
            PH_CHECK_SUCCESS_FCT(status, phalT1T_WriteNoEraseByte(
                pT1T->pAlT1TDataParams,
                pT1T->bUid,
                                             ...
                                             )
{
    ...
    else
    {
        ...
        {
            pT1T->wNdefHeaderAddr = pT1T->asLockCtrlTlv[pT1T->bLockTlvCount - 1].wOffset + 5U;
        }
        else if(pT1T->bMemoryTlvCount != 0U)
        {
            pT1T->wNdefHeaderAddr = pT1T->asMemCtrlTlv[pT1T->bMemoryTlvCount - 1].wOffset + 5U;
        }
        else
        {
        ...
        for(wIndex = 0; wIndex < pT1T->bLockTlvCount; wIndex++)
        {
            if((pT1T->asLockCtrlTlv[wIndex].wByteAddr / PHAL_TOP_T1T_SEGMENT_SIZE) == pT1T->sSegment.bAddress)
            {
                ...
                for(wOffset = wSegmentOffset;
                    wOffset < (wSegmentOffset + (pT1T->asLockCtrlTlv[wIndex].bSizeInBits / 8U));
                    wOffset++)
                {
        ...
        for(wIndex = 0; wIndex < pT1T->bMemoryTlvCount; wIndex++)
        {
            if((pT1T->asMemCtrlTlv[wIndex].wByteAddr / PHAL_TOP_T1T_SEGMENT_SIZE) == (pT1T->sSegment.bAddress))
            {
                ...
                for(wOffset = wSegmentOffset;
                    ...
                    wOffset++)
                {
                    pT1T->sSegment.bLockReservedOtp[wOffset / 8] |= (uint8_t)(1U << (wOffset % 8U));
                }
            }
        }
    }
...
{
    if((pT1T->sSegment.bLockReservedOtp[(wIndex % PHAL_TOP_T1T_SEGMENT_SIZE) / 8]) && (1U << (wIndex % 8U)))
    {
                                    ...
                                    )
{
    ...
    if(pT1T->bTagMemoryType != PHAL_TOP_T1T_TAG_MEMORY_TYPE_STATIC)
    {
        ...
        if((pT1T->wNdefMsgAddr % 8U) && (pT1T->wNdefMsgAddr == wAddress))
        {
    ...
    else
    {
        ...
        if((pT1T->bTerminatorTlvPresence == PH_ON) && ((pDataIndex + 1U) == pDataParams->wNdefLength))
        {
            ...
            for(wIndex = (wAddress + 1U); wIndex < ((pT1T->bTms + 1U) * 8U); wIndex++)
            {
                if(0u != (phalTop_Sw_Int_T1T_CheckLockReservedOtp(pT1T, wIndex)))
                {
                    PH_CHECK_SUCCESS_FCT(status, phalT1T_WriteEraseByte(
    ...
    if(0U != (bFirstBlock))
    {
        ...
        (void)memcpy(&aTempData[(wAddress % 8U) + 1U], &pData[pDataIndex], (8 - (wAddress % 8U)));
    ...
    else if(0U != (bLastBlock))
    {
        ...
        (void)memcpy(aTempData, &pData[pDataIndex], (size_t)(pDataParams->wNdefLength - pDataIndex));
        ...
        if(((pDataParams->wNdefLength - pDataIndex) == 8U) && (pT1T->bTerminatorTlvPresence == PH_ON))
        {
            ...
            /* Loop is needed, if current byte address is Lock, Reserved Memory
             * or OTP. Loop till we find data byte. */
            for(wIndex = ((wAddress / 8U) + 1U); (wIndex * 8U) < ((pT1T->bTms + 1U) * 8U); wIndex++)
            {
                if(0u != (phalTop_Sw_Int_T1T_CheckLockReservedOtp(pT1T, (wIndex * 8U))))
                {
                    PH_CHECK_SUCCESS_FCT(status, phalT1T_WriteEraseBlock(
                        pT1T->pAlT1TDataParams,
                        pT1T->bUid,
    ...
    while(wIndex < PHAL_TOP_T1T_SEGMENT_END_INDEX)
    {
        ...
        case PHAL_TOP_T1T_LOCK_CTRL_TLV:
            ...
            if(pT1T->bTagMemoryType != PHAL_TOP_T1T_TAG_MEMORY_TYPE_STATIC)
            {
                ...
                pT1T->asLockCtrlTlv[pT1T->bLockTlvCount].wByteAddr = (bPageAddr * (1U << bBytesPerPage)) + bByteOffset;
                pT1T->bLockTlvCount++;
        ...
        case PHAL_TOP_T1T_MEMORY_CTRL_TLV:
            ...
            if(pT1T->bTagMemoryType != PHAL_TOP_T1T_TAG_MEMORY_TYPE_STATIC)
            {
                ...
                pT1T->asMemCtrlTlv[pT1T->bMemoryTlvCount].wByteAddr = (bPageAddr * (1U << bBytesPerPage)) + bByteOffset;
                                        ...
                                        )
{
    ...
    *pTagState = PHAL_TOP_STATE_NONE;
    ...
    if((pT1T->wNdefHeaderAddr) && (pDataParams->wNdefLength != 0U))
    {
                                         ...
                                         )
{
       ...
       wIndex++)
    {
        ...
        aTlv[0] = 0x01;
        aTlv[1] = 0x03;
        aTlv[2] = (uint8_t)((uint16_t)(pT1T->asLockCtrlTlv[wIndex].wByteAddr / (1U << pT1T->asLockCtrlTlv[wIndex].bBytesPerPage)) << 4U) |
                  (uint8_t)(pT1T->asLockCtrlTlv[wIndex].wByteAddr % (1U << pT1T->asLockCtrlTlv[wIndex].bBytesPerPage));
       ...
       wIndex++)
    {
        ...
        aTlv[0] = 0x02;
        aTlv[1] = 0x03;
        aTlv[2] = (uint8_t)((uint16_t)(pT1T->asMemCtrlTlv[wIndex].wByteAddr / (1U << pT1T->asMemCtrlTlv[wIndex].bBytesPerPage)) << 4U) |
                  (uint8_t)(pT1T->asMemCtrlTlv[wIndex].wByteAddr % (1U << pT1T->asMemCtrlTlv[wIndex].bBytesPerPage));
    ...
    if((pT1T->bLockTlvCount !=0U) && (pT1T->bMemoryTlvCount != 0U))
    {
        pT1T->wNdefHeaderAddr =
            ...
            pT1T->asLockCtrlTlv[pT1T->bLockTlvCount - 1].wOffset :
            pT1T->asMemCtrlTlv[pT1T->bMemoryTlvCount - 1].wOffset) + 5U;
    }
    else if(pT1T->bLockTlvCount != 0U)
    {
        pT1T->wNdefHeaderAddr = pT1T->asLockCtrlTlv[pT1T->bLockTlvCount - 1].wOffset + 5U;
                                       ...
                                       )
{
    ...
    *pLength = 0;
    ...
    wTempIndex = 0;
    for(; wTempIndex < pDataParams->wNdefLength; wIndex++)
    {
