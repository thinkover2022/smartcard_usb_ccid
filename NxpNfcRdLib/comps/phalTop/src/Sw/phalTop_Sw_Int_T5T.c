                                          ...
                                          )
{
    ...
    {
        pT5T->pAlI15693DataParams = pDataParams->pTopTagsDataParams[pDataParams->bTagType - 1];
    }
    else
    {
        ...
        if(wIndex < PHAL_TOP_T5T_MAXIMUM_BLOCK_SIZE)
        {
            ...
            while(wIndex < PHAL_TOP_T5T_MAXIMUM_BLOCK_SIZE)
            {
                ...
                case PHAL_TOP_T5T_NDEF_TLV:
                    ...
                    if(((pT5T->wMlen * 8 /* */) - pT5T->wNdefHeaderAddr) > /* */ 0x102)
                    {
                                        ...
                                        )
{
    ...
    *pTagState = PHAL_TOP_STATE_NONE;
    ...
    if(((pDataParams->bVno & /* */ 0x0C) >> 2U) > (uint8_t)PHAL_TOP_T5T_MAJOR_VERSION)
    {
    ...
    if((pT5T->wNdefHeaderAddr) && (pDataParams->wNdefLength != 0U))
    {
    ...
    else if((pDataParams->wNdefLength == 0U) && (pT5T->wNdefHeaderAddr))
    {
        if(pT5T->bRwa == PHAL_TOP_T5T_CC_RWA_RW)
        {
                                       ...
                                       )
{
            ...
            wIndex++)
        {
            PH_CHECK_SUCCESS_FCT(status, phalTop_Sw_Int_T5T_Read(
                ...
                else
                {
                    ...
                    (void)memcpy(pData, (pRxBuffer + ((pT5T->wNdefMsgAddr) % (pT5T->bBlockSize))), (size_t)(wRxLength - ((pT5T->wNdefMsgAddr) % (pT5T->bBlockSize))));
    ...
    else
    {
        ...
        (void)memcpy(pData, (pData + ((pT5T->wNdefMsgAddr) % (pT5T->bBlockSize))), (wRxLength - ((pT5T->wNdefMsgAddr) % (pT5T->bBlockSize))));
    }
                                        ...
                                        )
{
    ...
    if((pT5T->wNdefHeaderAddr % (pT5T->bBlockSize)) == (pT5T->bBlockSize - 1))
    {
    ...
    if(wLength > 0xFEU)
    {
        ...
        if((pT5T->wNdefHeaderAddr % (pT5T->bBlockSize)) == (pT5T->bBlockSize - 1))
        {
            (void)memcpy(&aLengthTemp[3], &pData[0], (((pT5T->bBlockSize - 3) < wLength) ? (pT5T->bBlockSize - 3) : wLength));
            wBytesUsed =  pT5T->bBlockSize - 3U;
            wByteNum = 0;
        }
        /* Ndef Header lies at the second last byte and thus length info will split */
        else if((pT5T->wNdefHeaderAddr % (pT5T->bBlockSize)) == (pT5T->bBlockSize - 2))
        {
            (void)memcpy(&aLengthTemp[2], &pData[0], (((pT5T->bBlockSize - 2) < wLength) ? (pT5T->bBlockSize - 2) : wLength));
            wBytesUsed = pT5T->bBlockSize - 2U;
            wBlockNum++;
            bLengthSplit = 1;
        }
        /* Ndef Header lies at the third last byte and thus length info will split */
        else if((pT5T->wNdefHeaderAddr % (pT5T->bBlockSize)) == (pT5T->bBlockSize - 3))
        {
            (void)memcpy(&aLengthTemp[1], &pData[0], (((pT5T->bBlockSize - 1) < wLength) ? (pT5T->bBlockSize - 1) : wLength));
        ...
        else if((pT5T->wNdefHeaderAddr % (pT5T->bBlockSize)) == (pT5T->bBlockSize - 4))
        {
        ...
        {
            (void)memcpy(&aLengthTemp[(pT5T->wNdefHeaderAddr % (pT5T->bBlockSize)) + 4U], &pData[0],
    ...
    else
    {
        ...
        if((pT5T->wNdefHeaderAddr % (pT5T->bBlockSize)) == (pT5T->bBlockSize - 1))
        {
            (void)memcpy(&aLengthTemp[1], &pData[0], (size_t)(((pT5T->bBlockSize - 1U) < wLength) ? (pT5T->bBlockSize - 1U) : wLength));
            wBytesUsed = pT5T->bBlockSize - 1U;
            wByteNum = 0;
        }
        ...
        {
            (void)memcpy(&aLengthTemp[(pT5T->wNdefHeaderAddr % (pT5T->bBlockSize)) + 2U], &pData[0],
    ...
    if(wLength > wBytesUsed)
    {
        ...
        {
            (void)memcpy(&aTemp[0], &pData[((wLoopCount * pT5T->bBlockSize) + wBytesUsed)], (size_t)((wLength - wBytesUsed) % (pT5T->bBlockSize)));
            PH_CHECK_SUCCESS_FCT(status, phalTop_Sw_Int_T5T_Write(
                pT5T,
                (wBlockNum + 1U + wLoopCount),
                &aTemp[0],
    ...
    if(0U != (bLengthSplit))
    {
        ...
        if(wLength > 0xFEU)
        {
            if((pT5T->wNdefHeaderAddr % (pT5T->bBlockSize)) == (pT5T->bBlockSize - 2))
            {
    ...
    if( pDataParams->wMaxNdefLength > wLength)
    {
        ...
        if(((pT5T->wNdefHeaderAddr + ((wLength > 0xFEU) ? 3u : 1u) +  wLength) % (pT5T->bBlockSize)) == (pT5T->bBlockSize - 1))
        {
            wBlockNum++;
        }
                                                 ...
                                                 )
{
   ...
   uint16_t   PH_MEMLOC_REM wRxLength = 0;
    ...
    if((wIndex < (PHAL_TOP_T5T_MAXIMUM_BLOCK_SIZE - 1)) && (pData[wIndex + 1U] != 0xFFU))
    {
    ...
    else if ((wIndex == (PHAL_TOP_T5T_MAXIMUM_BLOCK_SIZE - 2)) && (pData[wIndex + 1U] == 0xFFU))
    {
        pLength[0] = pData[wIndex + 1U];
        PH_CHECK_SUCCESS_FCT(status, phalTop_Sw_Int_T5T_Read(
            pT5T,
    ...
    /* Get the length of the respective TLV if the tag field is at PHAL_TOP_T5T_MAXIMUM_BLOCK_SIZE-3
       and length is greater than 0xFF */
    else if ((wIndex == (PHAL_TOP_T5T_MAXIMUM_BLOCK_SIZE - 3)) && (pData[wIndex + 1U] == 0xFFU))
    {
        pLength[0] = pData[wIndex + 1U];
