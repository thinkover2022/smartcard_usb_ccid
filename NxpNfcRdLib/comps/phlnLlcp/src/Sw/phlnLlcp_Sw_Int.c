...
phStatus_t phlnLlcp_Sw_Int_CheckGenBytes(uint8_t *pGenBytes, uint16_t wLength, phlnLlcp_LMDataParams_t *pLMBytes)
{
    ...
    return phlnLlcp_Sw_Int_ParseGenBytes((uint8_t *)(pGenBytes + 3U), wLength - 3, pLMBytes);
}
    ...
    *pbUriLen = 0;
    ...
    while(wIndex < (wLength - 2))
    {
        ...
        case PHLN_LLCP_TLV_TYPE_SDREQ:
            *pbUriLen = pGenBytes[wIndex + 1U] - 1;
            if(pbTid != NULL)
            {
                *pbTid = pGenBytes[wIndex + 2U];
            }
            pUri = &pGenBytes[wIndex + 3U];
            wIndex += (3u + (*pbUriLen));
                                                              ...
                                                              uint8_t bIgnoreRsap)
{
    ...
    while ((pSockets != NULL) &&
        (((pSockets->bLsap != bLsap)) || ( (bIgnoreRsap != TRUE) && ((pSockets->bRsap != bRsap) && (pSockets->bRsap != 0x01U)))))
    {
                                                                 ...
                                                                 uint8_t bUriLen)
{
    ...
    while( pSockets!= NULL )
    {
        if(pSockets->pUri != NULL)
        {
            ...
            if (0 == memcmp(pSockets->pUri , pUri, strlen((char *)pSockets->pUri)))
            {
                break;
            }
        }
                                      ...
                                      uint16_t wLength)
{
    ...
    case PHLN_LLCP_PTYPE_SNL:
        ...
        {
            if (0 == (memcmp(pUri, aSDPURI, (sizeof(aSDPURI) - 1))))
            {
