    ...
    )
{
    ...
    /* Copy valid UID bits */
    wSndBytes = (uint16_t)(((bNvbUidIn & 0xF0U) >> 4U) + ((bNvbUidIn & 0x0FU) ? 1 : 0));
    ...
    /* Adjust Rx-Align */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(pDataParams->pHalDataParams, PHHAL_HW_CONFIG_RXALIGN, bNvbUidIn & 0x07U));

    /* Adjust TxBits */
    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(pDataParams->pHalDataParams, PHHAL_HW_CONFIG_TXLASTBITS, bNvbUidIn & 0x07U));
    ...
    if (bIsSelect == 0U)
    {
        ...
        /* Add new bitcount */
        bBitCount = (uint8_t)(((bNvbUidIn >> 4U) << 3U) + (wRcvBytes << 3U) + wRcvBits);
        ...
        *pNvbUidOut = (uint8_t)(((bBitCount >> 3U) << 4U) + (bBitCount & 0x07U));
        ...
        if (wRcvBytes > 0U)
        {
            ...
            {
                bCmdBuffer[2U + bUidStartIndex] = (bCmdBuffer[2U + bUidStartIndex] & (uint8_t)((1U << (bNvbUidIn & 0x07U)) - 1)) |
            ...
            (void)memcpy(&bCmdBuffer[2U + bUidStartIndex + 1U], &pRcvBuffer[1], (size_t)(wRcvBytes - 1u));
        }
        ...
        /* Copy UID */
        (void)memcpy(pUidOut, &bCmdBuffer[2], (size_t)(bUidStartIndex + wRcvBytes));
    }
