            ...
            */
            ...
            if (wIsoFrameLen > 0U)
            {
                ...
                PH_CHECK_SUCCESS_FCT(statusTmp, phpalI14443p4_Sw_BuildIBlock(
                    ...
                    pDataParams->bNad,
                    pDataParams->bPcbBlockNum,
                    (0U != (pDataParams->bStateNow & PHPAL_I14443P4_SW_STATE_CHAINING_BIT)) ? 1 : 0,
                    bIsoFrame,
                    &wIsoFrameLen));
                                        ...
                                        )
{
    ...
    do
    {
        ...
        if (bWtxm > 0U)
        {
            ...
            if(pDataParams->bOpeMode != RD_LIB_MODE_EMVCO)
            {
                dwTimeout = (uint32_t)(((PHPAL_I14443P4_SW_FWT_MIN_US * (1U << pDataParams->bFwi)) * bWtxm) + PHPAL_I14443P4_EXT_TIME_US);
            }
            else
            {
                dwTimeout = (uint32_t)((((PHPAL_I14443P4_SW_FWT_MIN_US) * (1U << pDataParams->bFwi) * bWtxm) + PHPAL_I14443P4_SW_DELTA_FWT_US + PHPAL_I14443P4_SW_DELTA_TPCD_US));
            }
            ...
            {
                PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(pDataParams->pHalDataParams, PHHAL_HW_CONFIG_TIMEOUT_VALUE_MS, (uint16_t)((dwTimeout / 1000) + 1U)));
            }
            else
            {
        ...
        if (0U == (bInvalidBlock))
        {
            ...
            if (0u != (PHPAL_I14443P4_SW_IS_I_BLOCK(pResp[PHPAL_I14443P4_SW_PCB_POS])))
            {
                ...
                else
                {
                    ...
                    /* 7.1.1.3 c), ISO/IEC 14443-4:2008(E), "During chaining the NAD shall only be transmitted in the first block of chain." */
                    if ((pDataParams->bNadEnabled) && (0U == ((pDataParams->bStateNow & PHPAL_I14443P4_SW_STATE_CHAINING_BIT))))
                    {
                        bCheckNad = 1;
                    }
                    else
