                                  ...
                                  )
{
    ...
    {
        if ((bFsdi > PHPAL_I14443P4A_EMVCO_FRAMESIZE_MAX) || (bCid > 14U))
        {
            return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_PAL_ISO14443P4A);
        }
    }
    ...
    if ((status & PH_ERR_MASK) == PH_ERR_SUCCESS)
    {
        ...
        if(pDataParams->bOpeMode == RD_LIB_MODE_ISO)
        {
            dwSfgt = (uint32_t)(PHPAL_I14443P4A_SW_FWT_MIN_US * (1U << bSfgi));
        }
        else
        {
            dwSfgt = (uint32_t)( (PHPAL_I14443P4A_SW_FWT_MIN_US + PHPAL_I14443P4A_EMVCO_SFGT_US) * (1U << bSfgi));
        }
        /* Perform SFGT Wait */
        if (dwSfgt > 0xFFFFU)
        {
        ...
        if(pDataParams->bOpeMode != RD_LIB_MODE_EMVCO)
        {
            dwFwt = (uint32_t)(PHPAL_I14443P4A_SW_FWT_MIN_US * (1U << pDataParams->bFwi));
        }
        else
        {
            dwFwt = (uint32_t)((PHPAL_I14443P4A_SW_FWT_MIN_US  * (1U << pDataParams->bFwi)) + PHPAL_I14443P4A_SW_DELTA_FWT_US + PHPAL_I14443P4A_SW_DELTA_TPCD_US);
        }
    ...
    else
    {
        ...
        else
        {
            ...
            /* S(DESELECT) failure */
            if (((statusTmp & PH_ERR_MASK) != PH_ERR_SUCCESS) || (wRespLength != 2U) || (pResp[0] != cmd[0]) || (pResp[1] != cmd[1]))
            {
                /* Set HLTA timeout */
                PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(
                    pDataParams->pHalDataParams,
