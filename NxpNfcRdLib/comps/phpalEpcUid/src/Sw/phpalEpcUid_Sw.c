                                     ...
                                     )
{
    ...
    if (bTagType == PHPAL_EPCUID_LABEL_EPC)
    {
        ...
        /* Copy EPC mask */
        (void)memcpy(pDataParams->LabelInfo.bEpc, pMask, (size_t)(bMaskBitLength >> 3U));
    }
    /* checks for UID */
    else if (bTagType == PHPAL_EPCUID_LABEL_UID)
    {
        ...
        /* Copy IDD mask */
        (void)memcpy(pDataParams->LabelInfo.bIdd, pMask, bMaskBitLength >> 3U);
    }
    else
    {
        return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_PAL_EPCUID);
    ...
    wCommandLen = (bBitCount == 0U) ? (bMaskBitLength >> 3U) : ((bMaskBitLength >> 3U) + 1U);
    ...
    if (0U != (bBitCount))
    {
        ...
        aCommand[wCommandLen - 1u] &= (uint8_t) (0xFFU << (8 - bBitCount));
                                        ...
                                        )
{
    ...
    if ((
        ...
        (wRxLength != (PHPAL_EPCUID_EPC_LENGTH + 2 - (pDataParams->bMaskBitLength >> 3U)))
        ) ||
        (
        (pDataParams->bLabelType == PHPAL_EPCUID_LABEL_UID) &&
        (wRxLength != (PHPAL_EPCUID_IDD_LENGTH + 2 - (pDataParams->bMaskBitLength >> 3U)))
        ))
    {
        pDataParams->bUidValid = PH_OFF;
        return PH_ADD_COMPCODE_FIXED(PH_ERR_PROTOCOL_ERROR, PH_COMP_PAL_EPCUID);
    ...
    {
        /* Copy EPC */
        (void)memcpy(&pDataParams->LabelInfo.bEpc[pDataParams->bMaskBitLength >> 3U], pRxBuffer, wRxLength - 2u);
    ...
    {
        /* Copy IDD */
        (void)memcpy(&pDataParams->LabelInfo.bIdd[pDataParams->bMaskBitLength >> 3U], pRxBuffer, wRxLength - 2u);
