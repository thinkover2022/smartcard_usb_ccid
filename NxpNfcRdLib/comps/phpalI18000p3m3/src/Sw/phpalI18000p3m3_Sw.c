                                     ...
                                     )
{
    ...
    for (bIndex = 0; bIndex < bMaskByteLength; ++bIndex)
    {
        ...
        if ((bIndex == (bMaskByteLength - 1)) && ((bMaskBitLength & 0x07U) > 0U))
        {
            bTmp &= (uint8_t)(0xFFU << (8 - (bMaskBitLength & 0x07U)));
        }
                                        ...
                                        )
{
    ...
    *(pSelectCmd + bCmdLength)    = bMaskBitLength << 2U;  /* 2 bits left */
    ...
    if (bTruncate != PH_OFF)
    {
        *(pSelectCmd + bCmdLength) |= (uint8_t)(0x80 >> (wCmdBitLength & 0x07U));
    }
    ++bCmdLength;
    ++wCmdBitLength;
    *bLen = bCmdLength;
    ...
    )
{
    ...
    do
    {
                ...
                if(pRxBuffer[wIndex] == 0U)
                {
                    if(0U == (bCardFound))
                    {
                        ...
                        if(pRxBuffer[wIndex + 1U] > (bPcLength + bPacketCrcLen))
                        {
                            ...
                            if((pRxBuffer[wIndex + 1U] - (bPcLength + bPacketCrcLen)) <= sizeof(aUii))
                            {
                                /* Copy UII */
                                (void)memcpy(
                                    aUii,
        ...
        if(0U == (bCardFound))
        {
            if(wTimeoutCount == ((uint32_t)1U << bQ))
            {
                /* Timeout */
