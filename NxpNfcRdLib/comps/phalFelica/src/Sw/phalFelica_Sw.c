                                        ...
                                        )
{
    ...
    if ((wRespLength < 1U) || (wRespLength != ((pRxBuffer[0] * 2U) + 1U)))
    {
        return PH_ADD_COMPCODE_FIXED(PH_ERR_PROTOCOL_ERROR, PH_COMP_AL_FELICA);
    }
    ...
    *pRxNumServices = pRxBuffer[0];
    (void)memcpy(pRxServiceList, &pRxBuffer[1], (size_t)(*pRxNumServices * 2U));
                              ...
                              )
{
    ...
    if (wRespLength != (3u + (16u * pRxBuffer[2])))
    {
        return PH_ADD_COMPCODE_FIXED(PH_ERR_PROTOCOL_ERROR, PH_COMP_AL_FELICA);
    }

    *pRxNumBlocks = pRxBuffer[2];
    (void)memcpy(pBlockData, &pRxBuffer[3], 16U * bTxNumBlocks);
