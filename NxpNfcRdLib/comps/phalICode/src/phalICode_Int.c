        ...
        uint16_t wNumBlocks, uint8_t * pData, uint16_t * pDataLen)
{
    ...
    if(0U != (bEnableBuffering))
    {
         ...
         * value.
         */
        wMaxNoBlocks = (uint8_t) (bOption ? PHAL_ICODE_MAX_BLOCKS_CM1_OPTION_FLAG_SET : PHAL_ICODE_MAX_BLOCKS_CM1_OPTION_FLAG_NOT_SET);
        ...
        do
        {
            ...
            *pDataLen += wRespLen;
            ...
            case PHAL_ICODE_CMD_EXTENDED_FAST_READ_MULTIPLE_BLOCKS:
                if((!bOption && ((wNumBlocks * 4U) == *pDataLen)) || (bOption && (((wNumBlocks * 4U) + wNumBlocks) == *pDataLen)))
                {
                    bAllBlocksRead = 1;
                }
                break;
        ...
        uint8_t bVerify, uint8_t bKeyNo, uint8_t bKeyVer, uint8_t ** ppResponse, uint16_t * pRespLen)
{
    ...
    if(0U != (bVerify))
    {
        ...
        {
            phalICode_Int_Reverse(aKey, sizeof(aKey));
        }
        ...
        uint16_t * pEasLen)
{
    ...
    if (bOption != PHAL_ICODE_OPTION_OFF)
    {
        wCmdLen = 2 + (uint16_t) (bEasIdMaskLen >> 3U);
        aCmdBuff[1] = bEasIdMaskLen;
        (void)memcpy(&aCmdBuff[2], pEasIdValue, wCmdLen - 2u);
    }
 ...
 *		bProtectionStatus		: New protection status.
 *									PHAL_ICODE_PROTECT_PAGE_PUBLIC
 ...
 *			Other Depending on implementation and underlaying component.
 */
phStatus_t phalICode_Int_ProtectPage(void * pPalSli15693DataParams, uint8_t bPageNo, uint8_t bProtectionStatus)
{
    phStatus_t  PH_MEMLOC_REM wStatus = 0;
    uint8_t     PH_MEMLOC_REM aCmdBuff[3];
    uint8_t *	PH_MEMLOC_REM pResponse = NULL;
 ...
 *			Other Depending on implementation and underlaying component.
 */
phStatus_t phalICode_Int_Destroy(void * pPalSli15693DataParams)
{
    phStatus_t  PH_MEMLOC_REM wStatus = 0;
    uint8_t     PH_MEMLOC_REM aCmdBuff[1];
    uint8_t *	PH_MEMLOC_REM pResponse = NULL;
    ...
    aCmdBuff[0] = PHAL_ICODE_CMD_DESTROY;
        ...
        uint16_t * pDataLen)
{
    ...
    aCmdBuff[bCmdLen++] = bNoOfBlocks - 1;
 ...
 *									0x01: PHAL_ICODE_PARAMETERS_TIMING_106_5_US
 ...
 *			Other Depending on implementation and underlaying component.
 */
phStatus_t phalICode_Int_ReadSRAM(void * pPalSli15693DataParams, uint8_t bOption, uint8_t bBlockNo, uint8_t bNumBlocks, uint8_t ** ppData,
        uint16_t * pDataLen)
{
    phStatus_t	PH_MEMLOC_REM wStatus = 0;
    uint8_t		PH_MEMLOC_REM aCmdBuff[3];
#if defined(NXPBUILD__PHHAL_HW_PN7462AU) || defined(NXPBUILD__PHHAL_HW_PN5180)
    uint8_t		PH_MEMLOC_REM aRespBuff[PHAL_ICODE_MAX_BLOCKS * PHAL_ICODE_BLOCK_SIZE];
    uint8_t *	PH_MEMLOC_REM pResponse = NULL;
    uint16_t	PH_MEMLOC_REM wRespLen = 0;
    uint8_t		PH_MEMLOC_REM bAllBlocksRead = 0;
    uint16_t	PH_MEMLOC_REM wCurrBlocksToRead = 0;
    uint16_t	PH_MEMLOC_REM wRemBlocksToRead = 0;
    ...
    /* Chaining if Reader is CM1/518/PN7462AU . CM1/518/PN7462AU reader supports maximum of 256 bytes read including headers and footers.
     * So if the number of bytes to read is more that the reader Read capability, error will be returned by HAL
     * and also chaining is not supported by Iso15693 specification. To overcome this issue with CM1 reader,
     * chaining is performed here.
     */

#if defined(NXPBUILD__PHHAL_HW_PN7462AU) || defined(NXPBUILD__PHHAL_HW_PN5180)

    bCurrBlockNo		= 0;
    wRemBlocksToRead	= bNumBlocks;
    wCurrBlocksToRead	= PHAL_ICODE_MAX_BLOCKS_CM1_OPTION_FLAG_NOT_SET;

    do
    {
        /* Frame ReadSRAM command information. */
        aCmdBuff[0] = PHAL_ICODE_CMD_READ_SRAM;
        aCmdBuff[1] = bCurrBlockNo;
        aCmdBuff[2] = (uint8_t) (wCurrBlocksToRead - 1u);
        ...
        /* Copy the received data to internal buffer. */
        (void)memcpy(&aRespBuff[wRespLen], pResponse, *pDataLen);
        wRespLen += *pDataLen;

        /* Update the variables to read the remaining data. */
        bCurrBlockNo += (uint8_t) (PHAL_ICODE_MAX_BLOCKS_CM1_OPTION_FLAG_NOT_SET);
        bAllBlocksRead = (uint8_t) (wRemBlocksToRead / PHAL_ICODE_MAX_BLOCKS_CM1_OPTION_FLAG_NOT_SET);
        wCurrBlocksToRead = wRemBlocksToRead % PHAL_ICODE_MAX_BLOCKS_CM1_OPTION_FLAG_NOT_SET;
        wRemBlocksToRead = wCurrBlocksToRead;

    }while(bAllBlocksRead != 0U);

    /* Copy the complete data to the interface response buffer. */
    *ppData = aRespBuff;
    *pDataLen = wRespLen;
    ...
    /* Adjust number of blocks. Adjustment is made because the User or the application will pass
     * the number of blocks starting from 1 to 256. But as per Iso15693 specification the number
     * of blocks ranges from 0 - 255.
     */
    --bNumBlocks;
 ...
 *			Other Depending on implementation and underlaying component.
 */
phStatus_t phalICode_Int_WriteSRAM(void * pPalSli15693DataParams, uint8_t bBlockNo, uint8_t bNumBlocks, uint8_t * pData, uint16_t wDataLen)
{
    phStatus_t	PH_MEMLOC_REM wStatus = 0;
    uint8_t		PH_MEMLOC_REM aCmdBuff[3];
    uint8_t *	PH_MEMLOC_REM pResponse = NULL;
    ...
    uint16_t	PH_MEMLOC_REM wCurrBlocksToWrite = 0;
    uint16_t	PH_MEMLOC_REM wRemBlocksToWrite = 0;
    uint8_t		PH_MEMLOC_REM bCurrBlockNo = 0;
    uint8_t		PH_MEMLOC_REM bCurrLen = 0;
    ...
    /* Chaining if Reader is CM1/PMN67462AU/PN5180. CM1/PMN67462AU/PN5180 reader supports maximum of 256 bytes write including headers and footers.
     * So if the number of bytes to write is more that the reader write capability, error will be returned by HAL
     * and also chaining is not supported by Iso15693 specification. To overcome this issue with CM1 reader,
     * chaining is performed here.
     */
#if defined(NXPBUILD__PHHAL_HW_PN7462AU) || defined(NXPBUILD__PHHAL_HW_PN5180)
    ...
    bCurrBlockNo		= 0;
    wRemBlocksToWrite	= bNumBlocks;
    wCurrBlocksToWrite	= PHAL_ICODE_MAX_BLOCKS_CM1_OPTION_FLAG_NOT_SET;

    do
    {
        /* Frame WriteSRAM command information. */
        aCmdBuff[0] = PHAL_ICODE_CMD_WRITE_SRAM;
        aCmdBuff[1] = bCurrBlockNo;
        aCmdBuff[2] = (uint8_t) wCurrBlocksToWrite;

        /* Update the current length variable. */
        bCurrLen = (uint8_t) (wCurrBlocksToWrite * PHAL_ICODE_BLOCK_SIZE);
        ...
        /* Update the variables to read the remaining data. */
        bCurrBlockNo += (uint8_t) (PHAL_ICODE_MAX_BLOCKS_CM1_OPTION_FLAG_NOT_SET);
        bAllBlocksWritten = (uint8_t) (wRemBlocksToWrite / PHAL_ICODE_MAX_BLOCKS_CM1_OPTION_FLAG_NOT_SET);
        wCurrBlocksToWrite = wRemBlocksToWrite % PHAL_ICODE_MAX_BLOCKS_CM1_OPTION_FLAG_NOT_SET;

        /* Update the Offset variable. */
        bOffset += bCurrLen;

    }while(bAllBlocksWritten != 0U);
...
phStatus_t phalICode_Int_GetTagType(void * pPalSli15693DataParams, uint16_t * pTagType)
{
    ...
    case PHAL_ICODE_TAG_TYPE_ICODE_PLUTUS:
        *pTagType = wTagType;
        break;
