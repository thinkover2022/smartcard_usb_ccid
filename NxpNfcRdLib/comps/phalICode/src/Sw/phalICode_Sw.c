        ...
        uint8_t bKeyNoCard)
{
    ...
    if(wRespLen != (1 /* Barker code. */ + 16 /* TAM1 Response */))
    {
        return PH_ADD_COMPCODE_FIXED(PH_ERR_PROTOCOL_ERROR, PH_COMP_AL_ICODE);
    }
    ...
    if(!((((pResponse[0]) & 0x80 /* Done Flag extraction. */) == 0x80U)		&&
            (((wRespFlag) & 0x02 /* Response buffer flag extraction */) == 0x02U)	&&
            (((wRespFlag) & 0x04 /* Final response flag extraction */) == 0x04U)))
    {
        return PH_ADD_COMPCODE_FIXED(PH_ERR_PROTOCOL_ERROR, PH_COMP_AL_ICODE);
    }
    ...
    (void)memcpy(&dwTRnd_TAM1, &aRespPlain[PHAL_ICODE_RANDOM_NUMBER_SIZE], 4);
    (void)memcpy(&wC_TAM1, &aRespPlain[PHAL_ICODE_RANDOM_NUMBER_SIZE + 4 /* TChallenge */], 2);
        ...
        uint8_t bKeyNoCard, uint8_t bPurposeMAM2)
{
    ...
    if(wRespLen != (1 /* Barker code. */ + 22 /* Rest of MAM1 response. */))
    {
        return PH_ADD_COMPCODE_FIXED(PH_ERR_PROTOCOL_ERROR, PH_COMP_AL_ICODE);
    }
    ...
    (void)memcpy(aTChallenge_MAM1, &pResponse[wRespLen - 5u], (PHAL_ICODE_RANDOM_NUMBER_SIZE - 4 /* TChallenge */));
    ...
    (void)memcpy(&wC_MAM1, &aRespPlain[PHAL_ICODE_RANDOM_NUMBER_SIZE + 4 /* TChallenge */], 2);
 ...
 *			Other Depending on implementation and underlaying component.
 */
phStatus_t phalICode_Sw_ProtectPage(phalICode_Sw_DataParams_t * pDataParams, uint8_t bPageNo, uint8_t bProtectionStatus)
{
    phStatus_t	PH_MEMLOC_REM wStatus = 0;

    PH_CHECK_SUCCESS_FCT(wStatus, phalICode_Int_ProtectPage(pDataParams->pPalSli15693DataParams, bPageNo, bProtectionStatus));
 ...
 *			Other Depending on implementation and underlaying component.
 */
phStatus_t phalICode_Sw_Destroy(phalICode_Sw_DataParams_t * pDataParams)
{
    phStatus_t	PH_MEMLOC_REM wStatus = 0;
 ...
 *			Other Depending on implementation and underlaying component.
 */
phStatus_t phalICode_Sw_WriteSRAM(phalICode_Sw_DataParams_t * pDataParams, uint8_t bBlockNo, uint8_t bNumBlocks, uint8_t * pData,
        uint16_t wDataLen)
{
    phStatus_t	PH_MEMLOC_REM wStatus = 0;
