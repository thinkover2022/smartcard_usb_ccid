            ...
            if (bStoredUidLength < PHPAL_SLI15693_UID_LENGTH)
            {
                ...
                if (0U != (pDataParams->bUidBitLength % 8U))
                {
                    if (bStoredUidLength < 7U)
                    {
                        ...
                        (void)memcpy(&(pDataParams->pUid[bStoredUidLength + 1U]), &pResp[1], (*pUidLength)-1);
                    }
                                     ...
                                     )
{
    ...
    if ((0U == ((wOption & PH_EXCHANGE_LEAVE_BUFFER_BIT))))
    {
        ...
        if ((pTxBuffer[0] > PHPAL_SLI15693_SW_CUSTOM_CMD_CODE_BEGIN) && (pTxBuffer[0] < PHPAL_SLI15693_SW_CUSTOM_CMD_CODE_END))
        {
            ...
            (pTxBuffer[0] == PHPAL_SLI15693_SW_CMD_EXTENDED_GET_SYSTEM_INFORMATION)
            &&
            (wTxLength > 1U)
            )
        {
        ...
        {
            /* Nothing special to be handled in this case */
        }
    ...
    )
{
    ...
    *pMoreCardsAvailable = 0;
    ...
    bMaskByteLength = (bMaskBitLength % 8U) ? ((bMaskBitLength >> 3U) + 1U) : (bMaskBitLength >> 3U);
            ...
            case PH_ERR_INTEGRITY_ERROR:
                ...
                *pMoreCardsAvailable = 1;
                ...
                else
                {
                    ...
                    aMaskBuffer[bMaskBitLength >> 3U] &= (uint8_t)(0xFF >> (8 - bUidLengthDummy));
    ...
    else
    {
        ...
        while (bMaskBitLength < PHPAL_SLI15693_SW_UID_COMPLETE)
        {
            ...
            case PH_ERR_IO_TIMEOUT:
                ...
                aMaskBuffer[(uint8_t)(bMaskBitLength - 1) >> 3U] = aMaskBuffer[(uint8_t)(bMaskBitLength - 1) >> 3U] ^ (uint8_t)(0x01U << (uint8_t)((bMaskBitLength - 1) % 8U));
                bTimeOutNotAllowed = 1;
                break;
                                      ...
                                      )
{
    ...
    case PHPAL_SLI15693_CONFIG_FLAGS:
         ...
         */
        if (0U != (wValue & PHPAL_SLI15693_FLAG_FAST_DATA_RATE))
        {
            ...
            {
                /* NFC mode supports only FAST HIGH Data Rate */
                if(pDataParams->bOpeMode == RD_LIB_MODE_NFC)
                {
                    return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_PAL_SLI15693);
                }
                                        ...
                                        )
{
    ...
    *pDataLength = 0;
    ...
    {
        bCmdBuffer[bCmdBufferLen - 1] &= (uint8_t)(0xFF >> (8 - (bMaskBitLength & 0x07U)));
    }
    ...
    if (bCmd != PHPAL_SLI15693_SW_CMD_INVENTORY)
    {
        ...
        if (!((bExtended) && (0U != ((bExtendedOptions & PHPAL_SLI15693_FLAG_SKIP_DATA)))))
        {
    ...
    if ((bExtended) && (0U != ((bExtendedOptions & PHPAL_SLI15693_FLAG_CID_RESPONSE))))
    {
        (void)memcpy(pCIDOut, pResp, 2);
    ...
    {
        if (0U != (bFlags & PHPAL_SLI15693_FLAG_OPTION))
        {
            /* The response length should not be less than the remaining UID */
            bStoredUidLength = pDataParams->bUidBitLength >> 3U;
            if((bExtended) && (0U != ((bExtendedOptions & PHPAL_SLI15693_FLAG_UID_MODE))))
            {
                *pUidLength = PHPAL_SLI15693_UID_LENGTH;
            }
            else
