                               ...
                               )
{
    ...
    /* Disable Emd Check in EmvCo mode. */
                                     ...
                                     )
{
    ...
    case PHAC_DISCLOOP_CONFIG_NEXT_POLL_STATE:
        if(!(((wValue & 0xFFU) == PHAC_DISCLOOP_POLL_STATE_DETECTION) ||
            ((wValue & 0xFFU) == PHAC_DISCLOOP_POLL_STATE_COLLISION_RESOLUTION) ||
            ((wValue & 0xFFU) == PHAC_DISCLOOP_POLL_STATE_REMOVAL)))
        {
            return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_AC_DISCLOOP);
        }
        pDataParams->bPollState = (phacDiscLoop_Sw_PollState_t)(wValue & 0xFFU);
        break;
