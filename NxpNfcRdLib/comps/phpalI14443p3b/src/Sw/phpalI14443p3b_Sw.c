                                        ...
                                        )
{
    ...
    /* Build the command */
    aCmd[0] = (uint8_t)(((uint8_t)(bSlotNumber-1) & 0x0FU) << 4U) | 0x05U;
                                   ...
                                   )
{
    ...
    (void)memcpy(&aCmd[1], pDataParams->pPupi, sizeof(pDataParams->pPupi));
                                    ...
                                    )
{
    ...
    if(pDataParams->bOpeMode != RD_LIB_MODE_EMVCO)
    {
        dwFwt = (uint32_t)((PHPAL_I14443P3B_SW_FWT_MIN_US * (1U << pDataParams->bFwi)) + PHPAL_I14443P3B_SW_EXT_TIME_US);
    }
    else
    {
        dwFwt = (uint32_t)((PHPAL_I14443P3B_SW_FWT_MIN_US  * (1U << pDataParams->bFwi)) + PHPAL_I14443P3B_SW_DELTA_FWT_US + PHPAL_I14443P3B_SW_DELTA_TPCD_US);

        /* Emvco: case_id TB201_0 TB201_1,TB201_2,TB201_3, TB110_4
         * Emvco Test equipment does not respond in time. Small delay added
         */
        if (pDataParams->bFwi <= 0x04U)
        {
            /* Emvco  case_id TB004_1 */
            dwFwt += (PHPAL_I14443P3B_SW_EXT_TIME_US * 2U);
        }
    }
    ...
    if ((pAtqb[11] & 0x01U) && ((pResp[0] & 0x0FU) != bCid))
    {
        return PH_ADD_COMPCODE_FIXED(PH_ERR_PROTOCOL_ERROR, PH_COMP_PAL_ISO14443P3B);
    }
    ...
    )
{
    ...
    /* Copy PUPI */
    (void)memcpy(pPupi, pDataParams->pPupi, sizeof(pDataParams->pPupi));
...
*/
    ...
    (void)memcpy( pDataParams->pPupi, &pAtqb[1], sizeof(pDataParams->pPupi) );
...
{
    (void)memcpy(pDataParams->pPupi, pPupi, sizeof(pDataParams->pPupi));
    pDataParams->bPupiValid = PH_ON;
